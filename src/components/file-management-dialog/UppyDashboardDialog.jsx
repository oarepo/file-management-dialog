...
const UppyDashboardDialog = ({
  ...
  useEffect(() => {
    ...
    uppy.setOptions({
      ...
      onBeforeUpload: (files) => {
        ...
        Object.keys(files).forEach((fileID) => {
          ...
          const fileNoteField = allowedMetaFields.find(
            (field) => field.id === "fileNote"
          );
          if (fileNoteField) {
            file.meta.fileNote =
              file.meta.fileNote ?? fileNoteField.defaultValue ?? "";
          }
      ...
      onBeforeFileAdded:
        !modifyExistingFiles && autoExtractImagesFromPDFs
          ...
          : (file, files) => {
              ...
              const hasDuplicateInUploadedFiles = uploadedFiles.some(
                (f) => f.name === file.name
              );

              if (hasDuplicateInUploadedFiles) {
                uppy.info(
                  uppy.i18n("noDuplicates", { fileName: file.name }),
                  "error",
                  5000
                );
              }
              ...
              return (
                !hasDuplicateInCurrentSession && !hasDuplicateInUploadedFiles
              );
            },
    });
    ...
    uppy.on("complete", (result) => {
      onCompletedUpload &&
        typeof onCompletedUpload === "function" &&
  ...
  useEffect(() => {
    ...
    */
    ...
    const addUppyFileObj = (fileSources) => {
      const file = fileSources.find(
        (file) => file.name === startEvent?.data?.file_key
      );
      const uppyFileObj = getTagFile(
        fileSources.find((file) => file.name === startEvent?.data?.file_key)
      );
      fetch(file.requestPath)
        ...
        .catch((error) => {
          ...
          uppy.info(
            {
              message:
                uppy.i18n("Error loading file with key") +
                ` ${startEvent?.data?.file_key}.`,
  ...
  }, [uppy, record.files, record.links?.files, modifyExistingFiles, locale]);

  useEffect(() => {
    uppy.getPlugin("OARepoUpload")?.setOptions({
      endpoint: record.files?.enabled
        ? record.links.files
        : record.files.links.self,
      allowedMetaFields: allowedMetaFields.map((field) => field.id),
      deleteBeforeUpload:
        modifyExistingFiles || startEvent?.event === "edit-file" ? true : false,
      locale: locale?.startsWith("cs") ? czechLocale : {},
    });
  }, [uppy, record.files, record.links?.files, locale]);
    ...
    if (window.Worker) {
      extractImageWorker.onmessage = (event) => {
        if (event.data?.type === "done") {
          isProcessing.current = false;
          uppy.info(
            `[${event.data.sourcePdf}] ${uppy.i18n(
              "Image extraction completed"
            )}. ${uppy.i18n("Image count")}: ${event.data.imageCount}`,
            "success",
            5000
          );
          return;
          ...
          uppy.info(
            {
              message: `${uppy.i18n("Error extracting images from")} ${
                event.data.sourcePdf
              }`,
  ...
  return (
    <>
      <DashboardModal
        ...
        metaFields={(file) => {
          const fields = [];
          allowedMetaFields.some((field) => field.id === "fileNote") &&
            fields.push({
              id: "fileNote",
          ...
          if (file.type.startsWith("image/")) {
            allowedMetaFields.some((field) => field.id === "caption") &&
              fields.push({
                id: "caption",
                name: manualI18n("Caption"),
                placeholder: manualI18n("Set the Caption here"),
              });
            allowedMetaFields.some((field) => field.id === "featured") &&
              fields.push({
                id: "featured",
...
UppyDashboardDialog.propTypes = {
  ...
  startEvent: PropTypes.shape({
    event: PropTypes.oneOf([
      "edit-file",
      "upload-file-without-edit",
      "upload-images-from-pdf",
    ]).isRequired,
